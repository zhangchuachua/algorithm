...
// *大致思路就是不合并数组，然后使用两个指针分别指向数组的第一个数，然后较小的指针都向后移动，直到指向对应的索引，因为数组都给了
// *所以中位数的索引也可以确定。这种方法的时间复杂度为O(m+n) 空间复杂度为O(1);
// *虽然思路说着很简单，但是实现起来一点都不简单，
// !我这里的思路不好，建议看下面哪个，这里应该根据中位数的规律来规定循环的次数更好一些。
function findMedianSortedArraysPointer(nums1, nums2) {
    // nums1的指针
    var nums1Index = -1;
    // nums2的指针
    var nums2Index = -1;
    ...
    var changeIndex = '';
    // 计算中位数的索引值，奇偶有不同的做法
    var mid = (nums1.length + nums2.length) / 2 - 1;
    var next = undefined;
    var res;
    if (Math.ceil(mid) === mid) {
    ...
    while (i < nums1.length + nums2.length) {
        ...
        if (next) {
            ...
            if (i === mid) {
                if (changeIndex === 'nums1Index')
                    res = nums1[nums1Index];
                else if (changeIndex === 'nums2Index')
                    res = nums2[nums2Index];
            }
            else if (i === next) {
...
// !代码简单了太多
function findMedianSortedArraysPointerOptimization(nums1, nums2) {
