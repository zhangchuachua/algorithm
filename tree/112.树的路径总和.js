...
function hasPathSumOptimization(root, targetSum) {
    // 如果为空就直接返回false
    if (!root)
        return false;
...
// !不是递归的做法
var hasPathSumNotRecursive = function (root, targetSum) {
    if (!root)
        return false;
    // 创建两个队列
    // 用来存储节点
    ...
    while (nodeQue.length > 0) {
        ...
        var root_1 = nodeQue.pop();
        var temp = valQue.pop();
        // 如果这个节点是叶子节点（没有左右孩子）
        if (!root_1.left && !root_1.right) {
            // 如何符合要求返回true 并 退出函数
            if (temp === targetSum)
                return true;
        ...
        if (root_1.left) {
            // 左孩子进队列
            nodeQue.unshift(root_1.left);
            // 保存此时路径总和
            valQue.unshift(root_1.left.val + temp);
        }
        // 有右孩子就进来
        if (root_1.right) {
            // 右孩子进队列
            nodeQue.unshift(root_1.right);
            // 保存此时路径总和
            valQue.unshift(root_1.right.val + temp);
        }
    }
