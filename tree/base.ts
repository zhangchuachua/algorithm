...
function postorderStack(root: Tree) {
  if (!root) return;
  const stack = [root];
  let reverse = []; // 使用一个数组来存储顺序
  while (stack.length) {
    const node = <Tree>stack.pop();
    reverse.push(node.val);
    if(node.left) stack.push(node.left);// 先入后出。
    if (node.right) stack.push(node.right);// *后序遍历先遍历右子树，那么就是先输出右子树，那么就后入，然后先出
  }
