...
function formatterNumber(num: number | string, n = 3) {
  ...
  const isNumRex = /^(?<prev>\d*)\.?(?<suf>\d*)$/;
  if (!isNumRex.test(s)) return false;
  const { groups } = s.match(isNumRex) as Match;
  let prev = '';
  let suf = '';
  if (s.includes('.')) {
    prev = groups.prev;
